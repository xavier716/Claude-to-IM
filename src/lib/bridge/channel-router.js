 ...
 */
export function createBinding(address, workingDirectory) {
    const { store } = getBridgeContext();
    ...
    const session = store.createSession(`Bridge: ${displayName}`, defaultModel, undefined, defaultCwd, 'code');
 ...
 */
export function bindToSession(address, codepilotSessionId) {
    const { store } = getBridgeContext();
