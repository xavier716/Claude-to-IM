...
// ── Frame builders ───────────────────────────────────────────────
export function buildIdentify(token, intents) {
    return {
        op: OP.IDENTIFY,
        d: {
            token: `QQBot ${token}`,
...
export function buildHeartbeat(lastSequence) {
    return { op: OP.HEARTBEAT, d: lastSequence };
}
export function buildResume(token, sessionId, seq) {
    return {
        op: OP.RESUME,
        d: {
            token: `QQBot ${token}`,
...
export function nextMsgSeq(inboundMsgId) {
    ...
    if (_seqMap.size > SEQ_MAP_MAX) {
        const keysToDelete = Array.from(_seqMap.keys()).slice(0, _seqMap.size - SEQ_MAP_MAX);
...
export async function sendPrivateMessage(accessToken, params) {
