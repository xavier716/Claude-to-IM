 ...
 */
export function hasComplexMarkdown(text) {
    // Fenced code blocks
    if (/```[\s\S]*?```/.test(text))
        return true;
 ...
 */
export function preprocessFeishuMarkdown(text) {
 ...
 */
export function buildCardContent(text) {
 ...
 */
export function buildPostContent(text) {
    return JSON.stringify({
        zh_cn: {
            content: [[{ tag: 'md', text }]],
        },
 ...
 */
export function htmlToFeishuMarkdown(html) {
    return html
        .replace(/<b>(.*?)<\/b>/gi, '**$1**')
 ...
 */
export function buildToolProgressMarkdown(tools) {
    if (tools.length === 0)
        return '';
 ...
 */
export function buildStreamingContent(text, tools) {
    let content = text || '';
 ...
 */
export function buildFinalCardJson(text, tools, footer) {
    const elements = [];
    // Main text content
    ...
    if (footer) {
        const parts = [];
        if (footer.status)
            parts.push(footer.status);
        if (footer.elapsed)
            parts.push(footer.elapsed);
        if (parts.length > 0) {
 ...
 */
export function buildPermissionButtonCard(text, permissionRequestId, chatId) {
    const buttons = [
